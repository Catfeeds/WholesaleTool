...
/**
 * 为了兼容新老数据而使用，有新路径的使用新路径，
 * 不然使用老路径，都没有使用一张默认图片替代
 * @param string $new_path
 * @param string $old_path
 * @return string
 */
function get_compare_img_path($new_path='',$old_path=''){
    $host = \think\Config::get('host_url');
    if($new_path){
        return $host.'upload/picture/'.$new_path;
    }elseif ($old_path){
        return $host.$old_path;
    }
    return $host.'static/module/admin/img/onimg.png';
}
