...
class NewOrder extends Common
{
    /**
     * @desc 终端下单接口
     * @author: Nicole.An
     * @time: 2018-11-08 10:50:00
     */
    public function create_order(){
        // $this->checkToken();
        $unionid = request()->param('unionId');
        $form_id = request()->param('form_id');
        $product_spec = Db::name("product_spec")-> column('name','id');
        //查询当前创建人是否是买家
...
//            dump(!empty($product_list));die;
            ...
            if(count($other_product_list)>0){
                ...
                foreach($other_product_list as $key => $value){
                    //判断是否是已有的联合利华商品
                    if($value['is_unliever'] === 0){
                        $jiale_product_id = Db::name("product") -> where(array("product_name"=>$value['product_name'],"spec_id"=>$value['spec_id'])) -> value('id');
                        $temp = [];
                        //查询产品信息
                        $product = Db::name('product')->where(['id'=>$jiale_product_id])->find();
                        //产品id  product_id
                        $temp['product_id'] = $jiale_product_id;
                        $temp['product_name'] = $product['product_name'];
                        $temp['sku_code'] = $product['sku_code'];
                        $temp['sku_unit'] = $product['sku_unit'];
                        //产品数量  product_count
                        ...
                        if($value['product_count'] === 0){
                            $this -> returnError([],400,"新增产品数量不能为0！");
                        }
                        //获取商品对照关系(以商品名称为准)
                        if($value['product_name']){
                            $product_parent = Db::connect('db_con')->name("product_parent") -> where(array("product_name"=>$value['product_name'],"supplier_id"=>$supplier_id,"created_by"=>$client_id))->find();
                            $product_parent_id = $product_parent['id'];
                            $by_supplier = $product_parent['by_supplier'];
                        }else{
                            $this -> returnError([],400,"新增产品名称不能为空！");
                        }
                        if(!$product_parent_id){
                            $product_parent = ['product_name'=>$value['product_name'],"supplier_id"=>$supplier_id,"created_by"=>$client_id,"by_supplier"=>0,"created_at"=>time(),"updated_at"=>time()];
                            $product_parent_id = Db::connect('db_con')->name('product_parent')->insertGetId($product_parent);
                            $by_supplier = 0;
                        }

                        //添加手动输入产品到产品表
                        $product_data = Db::connect('db_con')->name("product") -> where(array("product_name"=>$value['product_name'],"spec_id"=>$value['spec_id'],"created_by"=>$client_id)) -> find();
                        if($product_data){
                        ...
                        $temp['product_name'] = $value['product_name'];
                        $temp['sku_code'] = $value['sku_code'];
                        $temp['sku_unit'] = $product_spec[$value['spec_id']];//规格名称
                        $temp['product_count'] = $value['product_count'];
            ...
            foreach($detail as $key => $value){
                ...
                if($value['is_unlieve']==1){
                    ...
                    //新增产品同名且规格相同时将产品数量自增
                    $order_detail_id = Db::connect('db_con')->name("order_detail") -> where(array("product_name"=>$value['product_name'],"order_id"=>$value['order_id'],"sku_unit"=>$value['sku_unit'])) -> value('id');
                    if($order_detail_id){
                        $result = Db::connect('db_con')->name("order_detail")->where(['id' => $order_detail_id])->setInc('product_count', $value['product_count']);
                    }else{
